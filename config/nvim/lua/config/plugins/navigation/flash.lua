...
function Locations:fetch_line(line, start, backward, chars, range_back)
  ...
  local line_words = (
    backward
      and vim.iter(vim.iter(words(line:sub(1, range_back and -1 or start[2]))):totable()):rev()
  ...
  for word, pos in line_words do
    ...
    if label or partial then
      ...
      table.insert(Locations.list, Location(label[2], { start[1], pos + label[1] - 1 }, not partial))
    end
  end
end
...
function Locations:fetch(back, cursor)
  ...
  if back then
    lines = vim
      .iter(vim.api.nvim_buf_get_lines(0, math.max(0, cursor[1] - Locations.config.range), cursor[1], false))
      :rev()
      :totable()
  else
...
Char.setup = function(...)
  ...
  on_key_ns = vim.on_key(function(key)
    if (Char.state and key == Util.ESC and vim.fn.mode() == "n") or Char._active then
      Char._active = false
      Locations:clear()
      return
    end
